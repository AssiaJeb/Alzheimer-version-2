...
st.markdown(
    "*OASIS-2 · Day 4d Balanced Accuracy Ensemble · CNN + Tabular Models · Grad-CAM · SHAP*"
)
st.divider()
...
BALANCED_ENSEMBLE_DIR = MODELS_DIR / "balanced_ensemble"
DAY4D_WEIGHTS_PATH = BALANCED_ENSEMBLE_DIR / "day4d_weights.json"
DAY4D_CLASS_BIAS_PATH = BALANCED_ENSEMBLE_DIR / "day4d_class_bias.json"
...
@st.cache_resource
def load_day4d_ensemble_assets():
    """
    Load Day 4d balanced accuracy ensemble assets.
    """
    required = [
        ENSEMBLE_DIR / "rf_tabular_model.joblib",
        ENSEMBLE_DIR / "et_tabular_model.joblib",
        ENSEMBLE_DIR / "hgb_tabular_model.joblib",
        ENSEMBLE_DIR / "lr_tabular_model.joblib",
        DAY4D_WEIGHTS_PATH,
        DAY4D_CLASS_BIAS_PATH,
    ]

    for p in required:
        if not p.exists():
            raise FileNotFoundError(f"Missing Day 4d ensemble asset: {p}")

    assets = {
        "rf": joblib.load(ENSEMBLE_DIR / "rf_tabular_model.joblib"),
        "et": joblib.load(ENSEMBLE_DIR / "et_tabular_model.joblib"),
        "hgb": joblib.load(ENSEMBLE_DIR / "hgb_tabular_model.joblib"),
        "lr": joblib.load(ENSEMBLE_DIR / "lr_tabular_model.joblib"),
    }

    with open(DAY4D_WEIGHTS_PATH, "r") as f:
        weights = json.load(f)

    with open(DAY4D_CLASS_BIAS_PATH, "r") as f:
        bias_raw = json.load(f)

    bias = np.array(
        [bias_raw["CN"], bias_raw["MCI"], bias_raw["MA"]],
        dtype=np.float32
    )

    return assets, weights, bias


def predict_day4d_ensemble(img_batch_pp, tabular):
    """
    Final deployed prediction: Day 4d balanced accuracy ensemble.
    """
    tabular = tabular.astype(np.float32)

    assets, weights, bias = load_day4d_ensemble_assets()

    cnn_proba = model.predict([img_batch_pp, tabular], verbose=0)
    rf_proba = assets["rf"].predict_proba(tabular)
    et_proba = assets["et"].predict_proba(tabular)
    hgb_proba = assets["hgb"].predict_proba(tabular)
    lr_proba = assets["lr"].predict_proba(tabular)

    proba = (
        weights.get("cnn", 0.0) * cnn_proba +
        weights.get("rf", 0.0) * rf_proba +
        weights.get("et", 0.0) * et_proba +
        weights.get("hgb", 0.0) * hgb_proba +
        weights.get("lr", 0.0) * lr_proba
    )

    proba = proba * bias[None, :]
    proba = proba / (proba.sum(axis=1, keepdims=True) + 1e-8)
...
with st.sidebar:
    ...
    st.warning(
        ...
        "The deployed Day 4d ensemble achieved the highest overall accuracy among tested models, but MCI recall remains weak."
    )
...
if run_btn:
    ...
    with st.spinner("Preprocessing MRI and running model..."):
        ...
        proba = predict_day4d_ensemble(img_batch_pp, tabular)[0]
